...
def get_morgan2_only(mol): return get_morgan_only(mol, 2)
def get_morgan3_only(mol): return get_morgan_only(mol, 3)
def get_morgan4_only(mol): return get_morgan_only(mol, 4)

def get_morgan_combined(mol):
    m2, n2 = get_morgan2_only(mol)
    m3, n3 = get_morgan3_only(mol)
    m4, n4 = get_morgan4_only(mol)
    feats_list = [f for f in [m2,m3,m4] if f is not None]
    if len(feats_list)==0: return None, []
    return np.concatenate(feats_list), n2+n3+n4
